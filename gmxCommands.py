...
def cg2atConvert(cgFile, destinationPath):
    ...
    subprocess.run(['cg2at', '-c', f'{cgFile.absolute()}', '-gmx', 'gmx', '-loc', 'CG', '-ov', '0.25'], input=commands,text=True)
...
def ionize(initialFile, outputTPR, output,  ionsMDP, topology):
    # generate tpr file
    subprocess.run(['gmx', 'grompp', '-f', f'{ionsMDP}', '-c', f'{initialFile}', '-p', f'{topology}', '-o', f'{outputTPR}'])
    subprocess.run(['gmx', 'genion', '-s', f'{outputTPR}', '-o', f'{output}', '-p', f'{topology}', '-pname', 'SOD', '-nname', 'CLA', '-neutral'], input='5\n', text=True)
    # genion

def insert_molecules(insertMolecule, output, boxSize, number, radius, inputFile=None, writeFile=None):
    if inputFile != None:
        subprocess.run(['gmx', 'insert-molecules', '-f', f'{inputFile}', '-ci', f'{insertMolecule}',
        ...
        subprocess.run(['gmx', 'insert-molecules', '-ci', f'{insertMolecule}',
...
def center_setup(inputFile, outputFile, edgeDistance) -> None:
    subprocess.run(['gmx', 'editconf', '-f', f'{inputFile}', '-o', f'{outputFile}', '-c', '-box', f'{edgeDistance[0]}',f'{edgeDistance[1]}', f'{edgeDistance[2]}', '-pbc', 'yes', '-bt', 'cubic'])
    return None

def solvAA(inputFile, waterModel, output) -> None:
    subprocess.run(['gmx','solvate', '-cp', f'{inputFile}','-cs', f'{waterModel}.gro', '-o', f'{output}'])
    return None

def minimal_setup(inputFile, outputFile, edgeDistance) -> None:
    subprocess.run(['gmx', 'editconf', '-f', f'{inputFile}', '-o', f'{outputFile}', '-c', '-d',f'{edgeDistance}','-pbc', 'yes', '-bt', 'cubic'])
    return None
...
def packMol(inputFile_a, inputFile_b, outputFile, distance, parent) -> None:
    ...
    subprocess.run(['packmol', '-i', f'{inpF}'])

def solvate(inputFile, waterFile, outputFile, radius) -> None:
    subprocess.run(['gmx', 'solvate','-cp', f'{inputFile}', '-cs', f'{waterFile}', '-o', f'{outputFile}', '-radius', f'{radius}'])
    return None

def distance(inputFile, tprFile, indexFile, lipid, drug, output) -> None:
    subprocess.run(['gmx', 'distance', '-s', f'{tprFile}', '-f', f'{inputFile}', '-n', f'{indexFile}', '-select', f'com of group "{lipid}" plus com of group "{drug}"', '-oall', f'{output}'])
    return None

def convertPDB(inputFile, outputFile) -> None:
    subprocess.run(['gmx', 'editconf', '-f', f'{inputFile}', '-o', f'{outputFile}'])
    return None

def extractFrame(inputFile, tprFile, time, name, indexFile) -> None:
    subprocess.run(['gmx', 'trjconv', '-s', f'{tprFile}', '-f', f'{inputFile}', '-dump', f'{time}', '-o', f'{name}', '-n', f'{indexFile}'], input='0\n', text=True)
    return None

def makeTPR(inputFile, initialGeometry, topology, outputFile, index=None) -> None:
    if index == None:
        subprocess.run(['gmx', 'grompp', '-f', f'{inputFile}', '-c', f'{initialGeometry}', '-p', f'{topology}', '-o', f'{outputFile}' , '-maxwarn', '10', '-r', f'{initialGeometry}'])
    else:
        subprocess.run(['gmx', 'grompp', '-f', f'{inputFile}', '-c', f'{initialGeometry}', '-p', f'{topology}', '-o', f'{outputFile}', '-n', f'{index}', '-maxwarn', '10', '-r', f'{initialGeometry}'])
    return None

def runMD_CPU(defaultName, threads_omp):
    subprocess.run(['gmx', 'mdrun', '-deffnm', f'{defaultName}', '-ntomp', '4', '-v', '-ntmpi', '1', '-nb', 'gpu', '-pin', 'on', '-cpt', '5', '-pme', 'gpu'])
    return None

def runMD(defaultName, threads_mpi, pin, threads_omp):
    subprocess.run(['gmx', 'mdrun', '-deffnm', f'{defaultName}', '-ntmpi', f'{threads_mpi}', '-pin', f'{pin}', '-ntomp', f'{threads_omp}', '-v', '-nb', 'gpu'])
    return None

def runMDCPT(defaultName, threads_mpi, pin, threads_omp, cpt):
    subprocess.run(['gmx', 'mdrun', '-deffnm', f'{defaultName}', '-ntmpi', f'{threads_mpi}', '-pin', '{pin}', '-ntomp', f'{threads_omp}', '-cpi', f'{cpt}',  '-nb', 'gpu', '-cpt', '5', '-v'])
    return None

def makeIndex(reference, output, associations):
    # first make blank index file
    subprocess.run(['gmx', 'make_ndx', '-f', f'{reference}', '-o', f'{output}'],
                   input = 'q\n', text=True)
